...
with mp_pose.Pose(static_image_mode=1, min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    ...
    # while cap.isOpened():
        ...
        if key == ord('i'):  # 进入camera_mode
            cv2.destroyAllWindows()
            cv2.namedWindow('target', cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
            cv2.imshow('target', resized)  # 有原图像进行对比
            with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
                while cap.isOpened():
                    ...
                    if cv2.waitKey(10) & 0xFF == 27:
                        ...
                        break
            break
        elif key == ord('q'):  # 键盘输入q时完全退出
            cv2.destroyAllWindows()
            break
        # if cv2.waitKey(10000) & 0xFF == 27:  # 按Esc退出Static_read_mode
            # break
        # cv2.destroyAllWindows('target')
    # cap.release()
    # cv2.destroyAllWindows()
